...
def save_checkpoint(path: Path, model, optimizer, epoch: int, config, history: dict[str, list[float]] | None = None) -> None:
    torch.save(
        {
            "epoch": epoch,
...
def load_loss_history(plot_dir: Path) -> dict[str, list[float]]:
    ...
    return {
        "total_loss": list(history.get("total_loss", [])),
        "recon_loss": list(history.get("recon_loss", [])),
        "kl_loss": list(history.get("kl_loss", [])),
    }
